...
- **Derive types from authoritative schemas.** When a protocol buffer, OpenAPI spec, GraphQL schema, database migration, or design-system token file defines a shape, derive from it instead of hand-rolling a parallel type. For a transport type, "authoritative" means the concrete serialized endpoint boundary, not a broader domain or persistence model. Trace enrichment, persistence-time mutation, and serialization before copying optionality across that boundary. Manual duplication drifts. See the **encode-lessons-in-structure** principle skill.
