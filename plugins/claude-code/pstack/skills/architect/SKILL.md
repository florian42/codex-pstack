...
If concurrent candidates are unavailable, produce two structurally distinct
candidates serially and disclose that no independent agent comparison occurred.

Design before implementing. Sketch types, function signatures, class shapes, and module boundaries with `not implemented` bodies and pseudocode. Synthesize across multiple model perspectives, then fill in code against the chosen sketch. If implementation proves the sketch wrong, throw it out and redesign.

## Start

Track a plan with one entry per phase before starting. Autonomous mode without checkpoints needs the list to show phase position and keep phases from silently disappearing.
...
Use the current runtime's configured architect model roles. Never translate or
reuse model identifiers from the other runtime.
