...
Each live lane runs on its own cloud VM at the PR head. Drive through the runtime mapping's browser and CLI verification capability.
...
**Verify, live.** Tests alone are not sufficient verification. A PR is verified only when its unit, live, and perf boxes are all checked. Ten lanes on `<live-lane model>` at the PR head, per the boot recipe.
