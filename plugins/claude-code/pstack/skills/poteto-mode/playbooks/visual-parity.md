...
4. Verify each component against its baseline via image diff on the matching surface through the runtime's browser-verification capability. A nonzero diff is a fail; investigate the pixel delta, don't wave it through. Use bounded monitoring per component until the diff is zero.
