...
Read the [runtime contract](../../references/runtime/contract.md). Resolve the project skill destination through the current runtime's skill-authoring mapping and interactive browser driving through its browser mapping. Call the resolved destination `<project-skill-root>` below. If no skill-authoring capability is available, stop before creating files. If the app requires a browser and neither interactive control nor a repository harness is available, report the live proof as blocked; do not call the generated skill proven.
