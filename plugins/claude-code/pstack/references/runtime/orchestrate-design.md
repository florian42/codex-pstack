...
The existing Cursor plain-file helper remains unchanged behind its runtime profile. The Codex and Claude Code distributions package only `session-orch.ts`, `journal.ts`, and `program.ts` from the canonical skill source; the Claude Code profile relaxes the single-code-dispatch limit because its workers get isolated worktrees. It excludes tests, dependency installers, Graphite support, transcript utilities, cloud coordination, and dormant automation material.
