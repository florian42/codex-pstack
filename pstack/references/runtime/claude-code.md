...
| Select a model role | Prefer the parent session's model. Pass `model` on an `Agent` call only with an alias this install exposes (`opus`, `sonnet`, `haiku`, `fable`, or `inherit`), and only when the role benefits from an override. Cursor model files and slugs are ignored. If a requested alias is unavailable or substituted by policy, inherit the parent rather than translating names by guesswork. A multi-phase plan names its live-lane model with the alias its lanes run on, which is `inherit` unless the plan overrides it. |
