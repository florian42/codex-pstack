...
| Select a model role | Each spawn names a role line in `~/.cursor/rules/pstack-models.mdc`. If the rule or the line is missing, use the role's default below. Otherwise, set `model` to the line's value. When the value is `auto` or `inherit-parent`, leave `model` unset so the role runs on the parent model. An alias is never a rejected slug. If the `Task` tool rejects a configured slug, use the role's default and say so. For a panel entry (`arena runners`, `arena cross-judge pool`, `architect runners`, or `interrogate reviewers`), use instead the panel default of the same family. Families go by prefix: `claude-*`, `gpt-*`, and `grok-*`. With no family match, use the panel's first default. If the `Task` tool rejects a default, use the closest valid slug of the same family from its error message. Each code playbook reads the line that names it, such as `bug-fix`. The hardest changes read `hardest tasks`. Prose and judgment read `judgment and prose`. Architect reads `architect runners`, not `arena runners`. A multi-phase plan names its live-lane model with the `swarm workers` value. |
