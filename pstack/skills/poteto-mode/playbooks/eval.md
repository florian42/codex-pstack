...
6. **Verify the chain from transcripts, not self-report.** Read each candidate's own trace through the runtime mapping's delegate-trace row, which says where a delegate's trace lives, how it is keyed to the current session, and how to match it to a candidate's sanitized directory. Read only the traces of delegates this run spawned; never glob other workspaces or other sessions, that crosses workspace boundaries and reads private chats from unrelated projects. If the mapping records no trace for the way the candidates ran, grade chain-following as unverified and say so rather than falling back to self-report. Look at which files each candidate actually opened. Citing a principle is not reading its leaf skill, and reading it is not applying it. Grade chain-following from the files it really read plus the shape of the code, never from the candidate's own claims.
