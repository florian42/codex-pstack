...
1. Locate the prior trail. A prior session in the active workspace, a cloud-agent URL, or a pushed branch. Resolve a prior session through the runtime mapping's task-history row: the user names it by id, by a path they pasted, or as the most recent one, and you open only that session. Never glob other workspaces; that crosses workspace boundaries and reads private chats from unrelated projects. A cloud-agent URL is a source only where the mapping says how to read it; otherwise say so and fall back to the pushed branch. Read the metadata overview and last messages first, then scan back for the decision points. Parse a long transcript in a delegate through the runtime mapping and keep the reduced timeline in the main thread (the **principle-guard-the-context-window** skill).
