...
Read the [runtime contract](../../references/runtime/contract.md). Resolve the project skill root, delegation, monitoring, and browser-driving capabilities through the current runtime mapping. If no project skill root can be identified, stop without edits. If a required live surface cannot be driven, return `blocked`; source inspection alone is not a clean pass.
...
0. **Locate the target.** Find the verification skill to maintain under the runtime's project skill root: its body has launch/drive sections and a feature map. Several candidates → use the runtime's blocking-question capability; none → stop and point at `/create-verification-skill` instead of inventing a target.
