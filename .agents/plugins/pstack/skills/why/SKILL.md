...
Read the [runtime contract](../../references/runtime/contract.md). Discover
evidence capabilities from the current runtime's available tools and connectors,
then use its delegation, monitoring, and model-role mappings. An unavailable
evidence category is a named gap. Do not inspect another runtime's configuration
directory or claim an unavailable source was searched.
...
Before delegating investigators, list the evidence connectors and tools exposed by the current runtime. The available-tools map is authoritative. Do not inspect another runtime's configuration directory as a fallback.
...
Launch all matching investigators concurrently when the runtime supports it. One investigator per category lets each specialize in one tool's query vocabulary and result shape. Use the runtime's investigator model role. Investigators may need connector access, so forbid writes in the brief without selecting a mode that strips tools. If delegation is unavailable, query categories serially and disclose that no independent investigators ran.
...
Delegate one investigator per category that has a matching connector. Each owns exactly one evidence source.
...
Delegate one synthesizer using the runtime's strong synthesis role. Its quality check spot-verifies citations, so preserve evidence-connector access while forbidding writes in the brief. If delegation is unavailable, synthesize in the parent.
