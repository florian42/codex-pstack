...
- Several code units may be dispatched concurrently. Each code worker is an
  `Agent` call with `isolation: "worktree"` or an explicit worktree the
  coordinator created, so workers never share a writable checkout. The
  coordinator still integrates one unit at a time, fast-forward only.
